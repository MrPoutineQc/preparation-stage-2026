# Compétences
Cochez les compétences que vous aimeriez utiliser en stage. Ajoutez-en au besoin.:     
- [X] Designer, coder et publier des sites Web dynamiques    
- [ ] Réaliser et tourner des vidéos    
- [ ] Animer des créations 2D et 3D    
- [ ] Concevoir des compositions sonores et visuelles interactives    
- [X] Assembler des environnements de réalité virtuelle    
- [ ] Élaborer des spectacles augmentés    
- [ ] Exploiter les nouvelles technologies et leur potentiel créateur    
- [ ] Penser et optimiser l’expérience utilisateur    
- [ ] Créer des univers immersifs et interactifs    
- [ ] Collaborer avec diverses disciplines artistiques ou interdisciplinaires    


# Logiciels 
Cochez les logiciels avec lesquels vous êtes à l'aise. Ajoutez-en au besoin.:     
- [X] Visual Studio Code
- [X] Photoshop
- [x] Illustrator
- [ ] Lightroom
- [ ] Premiere
- [ ] Media Encoder
- [x] After Effects
- [x] Davinci Resolve
- [x] Maya
- [x] Unity
- [x] Reaper
- [ ] Ableton Live
- [ ] Max
- [ ] Arduino
- [ ] MadMapper
- [x] Microsoft Teams

# Langage de programmation
Cochez les langages avec lesquels vous êtes à l'aise. Ajoutez-en au besoin.:    
- [x] HTML
- [x] CSS
- [x] JavaScript
- [ ] PHP
- [ ] SQL
- [x] C# (Unity)
- [ ] C++ (Arduino)
- [x] Connaissance de systèmes de gestion de contenu (CMS)

# Objectif de carrière
Étudiant en Techniques d’intégration multimédia, je souhaite évoluer au sein d’une équipe dynamique partageant ma passion pour l’informatique et les technologies Web. À la suite de ma formation collégiale, je souhaite poursuivre mes études universitaires afin de devenir ingénieur logiciel dans les domaines de la cybersécurité, des systèmes intelligents ou de l’analyse de données.

# Projet 1 — Veyloria

## Informations générales

| Information                         | Détails               |
| ----------------------------------- | --------------------- |
| **Nom du projet**                   | Veyloria              |
| **Mention académique ou personnel** | Personnel             |
| **Réalisé dans le cadre du cours**  | Aucun                 |
| **Individuel ou en équipe**         | En équipe             |
| **Coéquipier**                      | Jérémy Thériault      |
| **Mon rôle**                        | Développeur Front-End |
| **Catégorie**                       | Web                   |

---

## Technologies utilisées

* **Éditeur :** Visual Studio Code
* **Langages :** HTML, SCSS, JavaScript
* **Framework / outils :** Tailwind CSS, GSAP

---

## Description du projet

### Résumé

Création d’un site web présentant une planète fictive.

### Objectif du projet

Nous avons décidé de réaliser ce projet afin d’approfondir nos connaissances en développement web et d’expérimenter différentes technologies.

### Ma contribution

J’ai réalisé l’intégration complète d’une page web en utilisant plusieurs technologies, notamment Tailwind CSS pour le responsive et le CSS personnalisé pour la mise en forme. J’ai également ajouté des animations interactives à l’aide de la bibliothèque GSAP afin d’améliorer l’expérience utilisateur.

---

## Documentation

* **GitHub :** https://github.com/MrPoutineQc/veyloria
* **Visuel que j'ai fait:**
![Alt text](https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/veyloria_card.png)

<br>

# Projet 2 — Critique de Counter-Strike 2

## Informations générales

| Information                         | Détails                      |
| ----------------------------------- | ---------------------------- |
| **Nom du projet**                   | Critique de Counter-Strike 2 |
| **Mention académique ou personnel** | Académique                   |
| **Réalisé dans le cadre du cours**  | Web 3                        |
| **Individuel ou en équipe**         | Individuel                   |
| **Coéquipier**                      | S/O                          |
| **Mon rôle**                        | Développeur Front-End        |
| **Catégorie**                       | Web                          |

---

## Technologies utilisées

* **Éditeur :** Visual Studio Code
* **Langages :** HTML, CSS
* **Framework / outils :** Bootstrap, Bootstrap Icons

---

## Description du projet

### Résumé

Création d’un site thématique d’évaluation et de critique du jeu *Counter-Strike 2*, intégrant la grille et différents composants de Bootstrap ainsi que Bootstrap Icons.

### Objectif du projet

Le mandat de ce travail pratique était d’élaborer un site Web sur une critique choisie par l’étudiant, tout en respectant une structure organisée et responsive à l’aide de la librairie Bootstrap.

### Ma contribution

J’ai conçu la mise en page complète autour de l’univers de *Counter-Strike 2* en exploitant les cartes, conteneurs et barres de navigation de Bootstrap. J’ai ensuite personnalisé les styles CSS afin de créer une identité visuelle adaptée au thème du jeu.

---

## Documentation

* **GitHub :** https://github.com/MrPoutineQc/zwarrren-tp1
* **Site web :** https://mrpoutineqc.github.io/zwarrren-tp1/

<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px;">

  <img src="https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/cs2_2.png" width="50%">
  <img src="https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/cs2_1.png" width="50%">

</div>

<br>

# Projet 3 — Qrypta

## Informations générales

| Information                         | Détails                                                  |
| ----------------------------------- | -------------------------------------------------------- |
| **Nom du projet**                   | Qrypta                                                   |
| **Mention académique ou personnel** | Académique                                               |
| **Réalisé dans le cadre du cours**  | Web 3                                                    |
| **Individuel ou en équipe**         | Individuel                                               |
| **Coéquipier**                      | S/O                                                      |
| **Mon rôle**                        | Designer d'interface & Développeur Front-End             |
| **Catégorie**                       | Interface Utilisateur (UI/UX) & Programmation interactive |

---

## Technologies utilisées

* **Éditeur :** Visual Studio Code
* **Langages :** HTML, CSS, JavaScript
* **Framework / outils :** Chart.js, Anime.js, ZDog, tone.js, vfx.js, maplibre

---

## Description du projet

### Résumé

Tableau de bord de style HUD futuriste intégrant de la visualisation de données dynamiques et animées.

### Objectif du projet

Le travail pratique consistait à développer une interface Web riche exploitant la manipulation avancée du DOM et des animations JavaScript. Il était demandé d'intégrer des bibliothèques externes pour générer des représentations graphiques interactives.

### Ma contribution

J'ai élaboré un design de type tableau de bord d’une banque du nom de Qrypta avec des animations CSS sur mesure. J'ai relié la bibliothèque Chart.js pour afficher des graphiques de données réactifs qui s'adaptent aux actions de l'utilisateur.

---

## Documentation
* **Moodboard:** https://www.figma.com/design/lBWtzsOKYsk4MhaqdRFQxy/WEB-3---TP2-HUD?node-id=0-1&t=GI8MiE8inmbPK1Z7-1
* **Github :** https://mrpoutineqc.github.io/qrypta/

![*Capture complète du tableau de bord finalisé en plein écran*](https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/qrypta_final.png)

<br>

# Projet 4 — Scootex

## Informations générales

| Information                         | Détails                                                  |
| ----------------------------------- | -------------------------------------------------------- |
| **Nom du projet**                   | Scootex                                                  |
| **Mention académique ou personnel** | Académique                                               |
| **Réalisé dans le cadre du cours**  | Web 4                                                    |
| **Individuel ou en équipe**         | Individuel                                               |
| **Coéquipier**                      | S/O                                                      |
| **Mon rôle**                        | Développeur & Intégrateur Web WordPress                  |
| **Catégorie**                       | Commerce électronique & Gestion de contenu (CMS)         |

---

## Technologies utilisées

* **Plateforme :** WordPress
* **Extensions / Moteurs :** WooCommerce, Elementor, Advanced Custom Fields (ACF)
* **Langages :** HTML, CSS

---

## Description du projet

### Résumé

Boutique e-commerce moderne et responsive dédiée à la vente de trottinettes électriques et d'équipements de mobilité urbaine.

### Objectif du projet

Le travail pratique consistait à concevoir et déployer une plateforme de commerce électronique fonctionnelle en exploitant un système de gestion de contenu. L'objectif était de structurer un catalogue de produits complet doté de métadonnées personnalisées et d'une expérience de navigation ergonomique.

### Ma contribution

J'ai conçu et mis en page l'ensemble des gabarits du site à l'aide d'Elementor tout en configurant la boutique via WooCommerce. J'ai également modélisé des champs dynamiques avec Advanced Custom Fields (ACF) pour afficher les fiches techniques détaillées de chaque modèle de trottinette.

---

## Documentation
https://202496312.tim-momo.com/warren_zackary_final/

![*Image page principale* ](https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/scootex_menu.png)

<br>

# Processus de création
## Processus de création étape par étape de Qrypta

### 1. Planification, idéation et direction artistique futuriste (HUD)

La première étape a consisté à définir le concept visuel et l'architecture fonctionnelle de l'interface bancaire afin de garantir une identité cyberpunk/sci-fi cohérente :

* **Définition de l'identité visuelle HUD :** Sélection d'un thème sombre avec des accents jaune-orange, typographies technologiques angulaires et dispositions modulaires inspirées des affichages tête haute de science-fiction.
* **Architecture de l'information :** Structuration des différents modules du tableau de bord (solde crypto, flux de transactions, indicateurs de performance, visualiseur 3D et métriques en temps réel).
* **Zonage et wireframing :** Élaboration d'une grille ergonomique permettant d'afficher une grande densité de données sans nuire à la lisibilité générale de la plateforme.

> ![*Moodboard*](https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/qrypta_moodbard.png)

---

### 2. Intégration de la structure et stylisation avancée (HTML5 & CSS3)

Mise en place du squelette de l'application web et création de l'atmosphère visuelle à l'aide de techniques CSS modernes :

* **Balisage sémantique HTML5 :** Découpage du tableau de bord en cartes autonomes (*widgets*) pour accueillir les graphiques, les listes de données et les contrôles interactifs.
* **Mise en page réactive (CSS Grid & Flexbox) :** Construction d'un layout adaptatif garantissant l'alignement parfait des différents panneaux quelle que soit la résolution d'écran.
* **Effets visuels avancés :** Utilisation intensive de propriétés graphiques (bordures lumineuses `box-shadow`, arrière-plans translucides avec `backdrop-filter: blur`, textures vectorielles en filigrane et animations de balayage).

> ![*Code du projets*](https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/qrypta_vscode.png)

---

### 3. Visualisation de données dynamiques (Chart.js)

Implémentation des modules graphiques pour transformer les données chiffrées en représentations visuelles claires et interactives :

* **Intégration de la bibliothèque Chart.js :** Instanciation de graphiques vectoriels sur des éléments HTML `<canvas>` pour afficher l'évolution des portefeuilles et les variations de marché.
* **Personnalisation esthétique des graphiques :** Remplacement des styles par défaut par des dégradés néon personnalisés, suppression des grilles encombrantes, ajustement des courbes de Bézier pour lisser les tracés et stylisation des infobulles (*tooltips*).
* **Gestion des métriques :** Structuration des séries de données pour simuler l'affichage d'actifs cryptographiques avec des mises à jour dynamiques au survol.

---

### 4. Animation d'éléments 3D et micro-interactions (Zdog & Anime.js)

Dynamisation du tableau de bord afin de renforcer l'aspect HUD interactif et futuriste :

* **Rendu pseudo-3D avec Zdog :** Conception et modélisation d'un élément géométrique tridimensionnel léger en canvas (comme un logo ou un noyau de données rotatif) tournant continuellement en arrière-plan.
* **Micro-interactions avec Anime.js :** Programmation d'effets d'apparition cinématiques au chargement de la page (déploiement en cascade des modules, compteurs de chiffres incrémentaux, pulsation des indicateurs d'état).
* **Manipulation événementielle du DOM :** Écoute des interactions de l'utilisateur (clics, survols, filtres temporels) en JavaScript pur pour actualiser instantanément l'état des composants sans rechargement de page.


**Vidéo de l'animation complète**

> [![Animation](https://img.youtube.com/vi/QQggqlVq3uM/maxresdefault.jpg)](https://youtu.be/QQggqlVq3uM)

---

### 5. Optimisation, tests d'affichage et stabilisation

Validation des performances d'exécution, du rendu visuel et de la réactivité de l'application :

* **Optimisation des performances :** Gestion prudente des boucles d'animation et de la fréquence de rafraîchissement des canvas pour éviter toute baisse de fluidité ).
* **Tests multi-résolutions :** Validation de la cohérence de l'interface sur différents ratios d'écran pour préserver l'équilibre visuel du HUD.
* **Nettoyage et documentation :** Modularisation des scripts JavaScript, minification des ressources et finalisation de la documentation technique du projet.

![> *Capture complète du tableau de bord finalisé en plein écran*](https://github.com/MrPoutineQc/preparation-stage-2026/blob/main/media/qrypta_final.png)


